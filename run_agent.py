...
def _schema_safe_external_tool_arguments(name: Any, args: Any) -> Dict[str, Any]:
    """Repair client-executed tool arguments before returning them to OpenCode."""
...
class AIAgent:
    ...
    def __init__(
        ...
        self._external_tool_mode = str(external_tool_mode or "none")
        self._tools_from_request = bool(tools)
        self._requested_tool_choice = tool_choice
        ...
        if self._external_tool_mode in {"broker", "inband"} and isinstance(tools, list):
    ...
    def _record_model_latency_if_slow(
        self,
        *,
        ...
        """Persist a short model cooldown when a provider is slow/hung.

        The gateway's swarm selector consults ``agent.model_cooldown_db`` before
        choosing providers.  Keep this best-effort and non-fatal: latency
        tracking must never break a successful model response.
        """
        provider = str(getattr(self, "provider", "") or "").strip()
        model_name = str(model or getattr(self, "model", "") or "").strip()
        effective_base = str(base_url or getattr(self, "base_url", "") or "").strip()
        if not provider or not model_name:
            return
        if effective_base and is_local_endpoint(effective_base):
            return
        try:
            from agent.model_cooldown_db import record_model_latency

            applied = record_model_latency(
                provider,
                model_name,
                elapsed_seconds,
                base_url=effective_base,
            ...
            if applied:
                logger.info(
                    "Slow model response recorded for cooldown: provider=%s model=%s elapsed=%.1fs reason=%s",
                    provider,
                    model_name,
                    elapsed_seconds,
                    reason,
            ...
            logger.debug("Failed to record model latency cooldown: %s", exc)
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        while t.is_alive():
            ...
            if _elapsed > _stale_timeout:
                ...
                self._record_model_latency_if_slow(
                    elapsed_seconds=_elapsed,
                    model=api_kwargs.get("model"),
                    reason="stale_non_stream_response",
                )
                self._emit_status(
        ...
        if result["error"] is not None:
            raise result["error"]
        self._record_model_latency_if_slow(
            elapsed_seconds=time.time() - _call_start,
            model=api_kwargs.get("model"),
            reason="slow_non_stream_response",
        )
    ...
    def _interruptible_streaming_api_call(
        ...
        if result["error"] is not None:
            ...
            raise result["error"]
        self._record_model_latency_if_slow(
            elapsed_seconds=time.time() - _stream_call_start,
            model=api_kwargs.get("model"),
            reason="slow_stream_response",
        )
        return result["response"]
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self.api_mode == "anthropic_messages":
            ...
            return _transport.build_kwargs(
                ...
                tools=self.tools,
                tool_choice=self._requested_tool_choice,
        ...
        if assistant_tool_calls:
            ...
            for tool_call in assistant_tool_calls:
                ...
                tc_dict = {
                    ...
                    "function": {
                        "name": tool_call.function.name,
                        "arguments": json.dumps(_schema_safe_external_tool_arguments(
                            tool_call.function.name,
                            tool_call.function.arguments,
                        ), ensure_ascii=False) if getattr(self, "_external_tool_mode", "none") in {"broker", "inband"} else tool_call.function.arguments,
                    },
                }
    ...
    def _execute_tool_calls(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        if getattr(self, "_external_tool_mode", "none") in {"broker", "inband"}:
            for tool_call in tool_calls:
                name = getattr(getattr(tool_call, "function", None), "name", "")
                args = getattr(getattr(tool_call, "function", None), "arguments", "{}")
                parsed = _schema_safe_external_tool_arguments(name, args)
                if self.tool_start_callback:
                    try:
                        self.tool_start_callback(tool_call.id, name, parsed)
                    except Exception as cb_err:
                        logging.debug(f"Tool start callback error: {cb_err}")
            self._touch_activity("external tool call pending")
            return
