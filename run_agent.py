...
class AIAgent:
    ...
    def __init__(
        ...
        # Override with external tools if provided (from API client like opencode)
        # Track which tools are from the client (should route execution back to client)
        self.client_tool_names = set()
        self.internal_tool_names = set()
        if tools is not None:
            self.tools = tools
            # Separate client tools from internal tools
            for tool in tools:
                tool_name = tool.get("function", {}).get("name", "")
                _fc = tool.get("_from_client", False)
                # When enabled_toolsets is empty AND tools are provided via API,
                # ALL tools are client tools (hermes-code mode - no Hermes tools loaded)
                _is_client_mode = (enabled_toolsets == [] and tools is not None)
                if _is_client_mode:
                    _fc = True
            ...
            print(f"DEBUG TOOL LOOP DONE: client={self.client_tool_names} internal={self.internal_tool_names}")
            # If bash is a client tool, terminal should also be treated as a client tool
            # since they're functionally similar (shell execution on client vs container)
            if "bash" in self.client_tool_names and "terminal" in self.internal_tool_names:
                self.client_tool_names.add("terminal")
                self.internal_tool_names.discard("terminal")
                print(f"DEBUG: Added terminal to client_tool_names (bash is client tool)")
            if not self.quiet_mode:
                print(f"🛠️  Using {len(tools)} external tools from API request")
        
        # Show tool configuration and store valid tool names for validation
        self.valid_tool_names = set()
        if self.tools:
            # Include both client and internal tools in valid names
            all_tool_names = {tool["function"]["name"] for tool in self.tools}
            self.valid_tool_names = all_tool_names
            tool_names = sorted(all_tool_names)
            if not self.quiet_mode:
                print(f"🛠️  Loaded {len(self.tools)} tools: {', '.join(tool_names)}")
                if self.client_tool_names:
                    print(f"   📱 Client tools (route back): {', '.join(sorted(self.client_tool_names))}")
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # DEBUG: Check tools being used
        print(f"  DEBUG: valid_tool_names = {getattr(self, 'valid_tool_names', 'NOT SET')}")
        print(f"  DEBUG: client_tool_names = {getattr(self, 'client_tool_names', set())}")
        print(f"  DEBUG: internal_tool_names = {getattr(self, 'internal_tool_names', set())}")
        print(f"  DEBUG: tools = {getattr(self, 'tools', 'NOT SET')}")
        if self.valid_tool_names:
            ...
            # Don't inject enforcement guidance when only client tools are present
            # - client tools should be routed back to opencode, not executed locally
            # - enforcement guidance causes qwen to use <tool_call> XML format instead of tool_calls array
            if _inject and hasattr(self, 'client_tool_names') and self.client_tool_names == self.valid_tool_names:
                _inject = False
                if not self.quiet_mode:
                    print(f"  ⏭️  Skipping enforcement guidance (client tools only)")
            if _inject:
                    ...
                    for event in stream:
                        ...
                        # get_final_response() returns an empty output list.
                            ...
                            if done_item is not None:
                                ...
                                self._maybe_fire_responses_tool_gen_started(done_item, notified_tool_call_ids)
    ...
    def _fire_tool_gen_started(self, tool_name: str, call_id: Optional[str] = None, arguments: str = "") -> None:
        ...
        if cb is not None:
            try:
                cb(tool_name, call_id, arguments)
            except Exception:
                pass

    def _maybe_fire_responses_tool_gen_started(self, item: Any, notified_ids: set[str]) -> None:
        """Emit tool-gen callback for Responses API function_call items."""
        item_type = getattr(item, "type", None)
        ...
        call_id = call_id.strip()
        if call_id in notified_ids:
            return
        notified_ids.add(call_id)
        self._fire_tool_gen_started(fn_name, call_id, arguments)
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            def _emit_completed_tool_calls(force: bool = False) -> None:
                """Emit tool-call callback only after arguments are complete."""
                ready_reasons = {"tool_calls", "function_call"}
                for idx in sorted(tool_calls_acc):
                    entry = tool_calls_acc[idx]
                    name = entry.get("function", {}).get("name", "")
                    if not name or idx in tool_gen_notified:
                        continue
                    if not force and finish_reason not in ready_reasons:
                        continue
            ...
            for chunk in stream:
                ...
                if chunk.choices[0].finish_reason:
                    ...
                    _emit_completed_tool_calls()

                # Usage in the final chunk
                if hasattr(chunk, "usage") and chunk.usage:
                    usage_obj = chunk.usage

            _emit_completed_tool_calls(force=bool(tool_calls_acc))
                ...
                for event in stream:
                    ...
                    if event_type == "content_block_start":
                        ...
                        if block and getattr(block, "type", None) == "tool_use":
                            ...
                            if tool_name:
                                _fire_first_delta()
                                self._fire_tool_gen_started(tool_name, tool_id)
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self.tools:
            # Strip _from_client flag before sending to provider (keeps internal routing but clean for API)
            clean_tools = []
            for tool in self.tools:
                clean_tool = {k: v for k, v in tool.items() if k != "_from_client"}
                clean_tools.append(clean_tool)
            api_kwargs["tools"] = clean_tools
    ...
    def _await_external_tool_result(self, tool_call, function_name: str, function_args: dict, index: int) -> tuple[str, bool, str]:
        ...
        logger.info("Waiting for external tool result session=%s call_id=%s tool=%s", session_id, call_id, function_name)
        ...
        if not waited:
            logger.warning("External tool timeout session=%s call_id=%s tool=%s timeout=%ss", session_id, call_id, function_name, timeout)
            return f"[Timeout waiting for external tool '{function_name}' result (call_id: {call_id})]", True, call_id

        posted = pending.result
        if pending.status == "ok":
            logger.info("External tool completed session=%s call_id=%s tool=%s", session_id, call_id, function_name)
        ...
        logger.warning("External tool returned error session=%s call_id=%s tool=%s", session_id, call_id, function_name)
        if isinstance(posted, str):
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        def _run_tool(index, tool_call, function_name, function_args):
            ...
            is_error = False
            try:
                # Broker/provider mode: when tools were supplied by the API
                # client, do NOT execute them locally.  Instead wait for the
                # client to POST the tool result to the hub.
                broker_mode = getattr(self, "_external_tool_mode", "none") == "broker"
                if broker_mode:
                    result, is_error, resolved_call_id = self._await_external_tool_result(tool_call, function_name, function_args, index)
                    if resolved_call_id and not getattr(tool_call, "id", None):
            ...
            if is_error:
                logger.info("tool %s failed (%.2fs): %s", function_name, duration, (result or '')[:200])
            else:
                logger.info("tool %s completed (%.2fs, %d chars)", function_name, duration, len(result or ""))
            results[index] = (function_name, function_args, result or "", duration, is_error)
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            if hasattr(self, 'client_tool_names') and function_name in self.client_tool_names:
                ...
                pending_msg = {
                    ...
                    "content": f"[Tool '{function_name}' queued for client execution]",
                    "_pending_client_execution": True,
                    "function_name": function_name,
                    "function_arguments": tool_call.function.arguments,
                }
                messages.append(pending_msg)
                if not self.quiet_mode:
                    print(f"  📱 Client tool (routed to client): {function_name}")
                continue  # Skip local execution, let client handle it
            ...
            if broker_mode:
                function_result, _is_error_result, call_id = self._await_external_tool_result(tool_call, function_name, function_args, i - 1)
                try:
                    tool_call.id = call_id
                except Exception:
                    pass
    ...
    def run_conversation(
        ...
        has_new_user_message = isinstance(user_message, str) and bool(user_message.strip())
        ...
        # Add user message unless this is an in-band tool continuation turn.
        current_turn_user_idx = None
        if has_new_user_message or not messages:
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    if getattr(self, "_external_tool_mode", "none") == "inband":
                        logger.info("Returning tool calls to client for in-band continuation: session=%s tool_count=%d", self.session_id or "none", len(assistant_message.tool_calls))
                        self._persist_session(messages, conversation_history)
                        return {
