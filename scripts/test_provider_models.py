...
Run:
    python3 scripts/test_provider_models.py [--provider filter]
"""
...
TOOLS = [{"type": "function", "function": {"name": "read_file", "description": "Read a file",
  ...
  {"type": "function", "function": {"name": "list_directory", "description": "List a directory",
  "parameters": {"type": "object", "properties": {"path": {"type": "string", "description": "Dir path"}}, "required": ["path"]}}}]

TOOL_MSGS = [{"role": "system", "content": "Use tools to answer."},
             {"role": "user", "content": "List files in /tmp"}]
SIMPLE_MSGS = [{"role": "user", "content": "Reply: 42"}]


def gateway_url():
    return os.getenv("HERMES_HOST", "http://0.0.0.0:8642")

def call_gw(model, messages, tools=None, timeout=60):
    body = {"model": model, "messages": messages, "max_tokens": 256, "temperature": 0}
    if tools: body["tools"] = tools
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {os.getenv('API_SERVER_KEY', '')}"}
    req = urllib.request.Request(f"{gateway_url()}/v1/chat/completions", data=json.dumps(body).encode(), headers=headers, method="POST")
    start = time.time()
    try:
        resp = urllib.request.urlopen(req, timeout=timeout)
        r = {"ok": True, "latency_ms": (time.time()-start)*1000, "body": json.loads(resp.read()), "status": resp.status}
    except urllib.error.HTTPError as e:
        r = {"ok": False, "latency_ms": (time.time()-start)*1000, "status": e.code, "body_raw": "failed"}
        try: r["body_raw"] = e.read().decode(errors="replace")[:500]
        except: pass
    except Exception as e:
        r = {"ok": False, "latency_ms": (time.time()-start)*1000, "error": str(e)[:200]}
    return r

def test_tc(model):
    r = call_gw(model, TOOL_MSGS, TOOLS, 60)
    if not r.get("ok"): return {"tc": False, "err": r.get("body_raw","") or r.get("error",""), "ms": r["latency_ms"], "sc": r.get("status",0)}
    c = r["body"].get("choices",[])
    if not c: return {"tc": False, "err": "no choices", "ms": r["latency_ms"]}
    m = c[0].get("message",{})
    tcs = m.get("tool_calls",[]); ct = (m.get("content","") or ""); fn = c[0].get("finish_reason","stop")
    return {"tc": bool(tcs) or fn=="tool_calls", "text_only": not bool(tcs or fn=="tool_calls") and bool(ct),
            "tools": [x.get("function",{}).get("name","?") for x in (tcs or [])],
            "ct": ct[:200], "ms": r["latency_ms"]}

def test_txt(model):
    r = call_gw(model, SIMPLE_MSGS)
    if not r.get("ok"): return {"ok": False, "err": r.get("body_raw","") or r.get("error",""), "ms": r["latency_ms"]}
    c = (r["body"].get("choices",[{}])[0].get("message",{}).get("content","") or "")
    return {"ok": bool(c.strip()), "ct": c[:200], "ms": r["latency_ms"]}
...
def get_models():
    ...
    if key:
        try:
            ...
            data = json.loads(urllib.request.urlopen(req, timeout=15).read()).get("data",[])
            for m in data:
            ...
            print(f"[WARN] OR fail: {e}", file=sys.stderr)
    return models
...
def test_one(entry):
    ...
    status, detail = "FAIL", "?"
    if tc["tc"]: status, detail = "TOOLS_OK", f"tools: {','.join(tc.get('tools',[]))[:40]}"
    elif tc.get("text_only"): status, detail = "TXT_ONLY", "text-only"
    ...
    try:
        ...
        if tc["tc"]: record_success(prov, model, latency_ms=ms)
        elif tc.get("text_only"): record_text_only(prov, model, latency_ms=ms)
        else: record_failure(prov, model, latency_ms=ms, error_code=tc.get("sc",0), error_message=tc.get("err",""))
    except: pass
    return {"model": model, "status": status, "latency_ms": ms, "tc": tc, "txt": st}
...
def main():
    ...
    print(f"{'='*100}\nGateway Model Test — {len(models)} models\n{'='*100}")
    results = []
    for src, ms in sorted(srcs.items()):
        ...
        for e in ms:
            try: results.append(test_one(e))
                ...
                print(f"  {e['model']:70s} ERROR  {x}")
                results.append({"model": e["model"], "status": "ERROR", "error": str(x)})
