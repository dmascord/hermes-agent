...
class TestExplicitModelRuntimeAlignment:
    ...
    def test_create_agent_explicit_prefixed_model_with_mismatched_configured_provider(self):
        adapter = APIServerAdapter(PlatformConfig(enabled=True))

        fake_agent = object()
        with (
            patch.dict("os.environ", {"HERMES_CODE_MODEL": "github-copilot-enterprise/claude-sonnet-4.6"}, clear=False),
            patch("gateway.run._load_gateway_config", return_value={"model": {"provider": "zai"}}),
            patch("gateway.run._resolve_runtime_agent_kwargs", return_value={
                "provider": "zai",
                "base_url": "https://api.z.ai/api/coding/paas/v4",
                "api_key": "zai-key",
            }),
            patch("gateway.platforms.api_server._align_runtime_with_explicit_model", return_value={
                "provider": "copilot",
                "base_url": "https://copilot-api.sita.ghe.com",
                "api_key": "gho_enterprise",
                "api_mode": "anthropic_messages",
            }),
            patch("gateway.run._resolve_gateway_model", return_value="ignored-default"),
            ...
            result = adapter._create_agent(session_id="api-session")

        assert result is fake_agent
        kwargs = mock_agent.call_args.kwargs
        assert kwargs["model"] == "claude-sonnet-4.6"
        assert kwargs["provider"] == "copilot"
        assert kwargs["base_url"] == "https://copilot-api.sita.ghe.com"
