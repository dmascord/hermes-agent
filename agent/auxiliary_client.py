...
_API_KEY_PROVIDER_AUX_MODELS: Dict[str, str] = {
    ...
    "minimax": "MiniMax-M2.1",  # Fastest minimax model for compression
    "minimax-oauth": "MiniMax-M2.1-highspeed",
    ...
    "ollama-cloud": "gemma3:4b",  # Fastest ollama-cloud model for compression
    "synthetic": "hf:deepseek-ai/DeepSeek-V3",  # Fast DeepSeek via HuggingFace endpoints
    "xiaomi": "mimo-v2.5-pro",  # Xiaomi MiMo compression model
...
# Compression/summary model chain.
#
# Ordered by priority: tried left-to-right, first available client wins.
# Each entry is a (label, try_fn) where try_fn() returns (client, model) or
# (None, None).  Compression is a high-volume side task so we prefer free
# models with generous rate limits; paid/configured providers are last resorts.
#
# NOT included: Copilot Enterprise (github-copilot-enterprise).  Copilot
# returns "unauthorized" for non-chat tasks (compression, summarization) even
# with valid credentials — it genuinely does not support this task type.
_COMPRESSION_PROVIDER_CHAIN: list[tuple[str, callable]] = [
    ("openrouter", lambda: _try_openrouter()),
    ("nous", lambda: _try_nous()),
    ("custom", lambda: _try_custom_endpoint()),
    ("api-key", lambda: _resolve_api_key_provider()),
]
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        try:
            _adapter_start = time.time()
            _adapter_provider = "copilot"
            _adapter_model = model
...
class CodexAuxiliaryClient:
    ...
    def __init__(self, real_client: OpenAI, model: str):
        ...
        # Preserve default_headers for introspection by callers that check _default_headers
        self._default_headers = getattr(real_client, "_default_headers", {}) or getattr(real_client, "_client", None) and dict(real_client._client.headers) or {}
...
def _resolve_api_key_provider_for(
    provider_id: str,
) -> Tuple[Optional[OpenAI], Optional[str]]:
    """Resolve a specific api-key provider by ID (e.g. "minimax", "ollama-cloud").

    Returns (client, model) for the provider if it has usable runtime credentials
    and a known compression-suitable model, or (None, None) otherwise.
    """
    try:
        from hermes_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
        logger.debug("Could not import PROVIDER_REGISTRY")
        return None, None
    ...
    if pool_present:
        if entry is None:
            # Pool exists but has no available entries (all exhausted or
            # slow_response).  Fall through to direct credential read so we
            # still get a client if the env var / auth store has credentials.
            pass
        else:
            api_key = _pool_runtime_api_key(entry)
            if not api_key:
...
def _build_codex_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if pool_present:
        ...
        if codex_token:
            ...
            # Pool entry has no runtime key (e.g. stale exhausted entry from a
            # previous run whose state was wiped).  Fall back to the direct auth
            # store read which reads providers["openai-codex"].tokens — this is
            # where `hermes auth add codex` stores tokens and is the correct
            # fallback for the raw_codex path used by fallback activation.
...
def _is_server_error(exc: Exception) -> bool:
    """Detect 5xx server errors that warrant trying the next provider.

    Returns True for HTTP 500 / 502 / 503 responses.  These indicate the
    provider's server is having trouble (overloaded, cold-start, etc.) rather
    than a problem with the request itself.  The caller should treat this as a
    transient failure and back off before retrying the same provider.
    """
    status = getattr(exc, "status_code", None)
...
def _is_model_unavailable_error(exc: Exception) -> bool:
    """Detect provider/model mismatches that should trigger auto fallback.

    Some auxiliary calls inherit the main runtime model during ``provider=auto``.
    For hermes-code this can surface provider-specific rejections like
    ``Model opencode-go/deepseek-v4-pro not supported`` or ``model not found``.
    Those are not transport failures, but for auto routing they are still safe
    to treat as recoverable by falling through to the normal auxiliary chain.
    """
    status = getattr(exc, "status_code", None)
    err_lower = str(exc).lower()

    # Catch 4xx errors that indicate the request itself is flawed (bad model
    # name, unsupported parameter, etc.) — these should always try the next
    # provider rather than looping on the same endpoint.
    if status in (400, 401, 404, 422, 503):
        if any(marker in err_lower for marker in (
            ...
            return True
        # For 400/422 with no keyword match, also treat as unavailable —
        # "Param Incorrect" from Ollama, "invalid request" from other
        # providers, etc. are all non-retryable at the model level.
...
def resolve_provider_client(
    ...
    def _pick_copilot_pool_entry(model_name: Optional[str]) -> Optional[Any]:
        ...
        Also checks ``main_runtime.base_url`` — when the caller (api_server)
        already resolved a GHE base URL (e.g. from a ``github-copilot-enterprise/``
        model), use that to select the matching pool entry so the normalized
        model string (``gpt-5.4``) still routes to GHE credentials.
        """
        try:
        ...
        # Priority 1: main_runtime base_url — if caller already resolved a GHE URL
        # (e.g. api_server set it for github-copilot-enterprise/*), match it.
        if main_runtime and main_runtime.get("base_url"):
            runtime_base = main_runtime["base_url"].strip().rstrip("/").lower()
            if "copilot-api." in runtime_base:
    ...
    if pconfig.auth_type == "api_key":
        ...
        raw_base_url = str(creds.get("base_url", "")).strip().rstrip("/") or pconfig.inference_base_url
        # Copilot pool selection: either source=credential_pool (normal case)
        # OR the token is a GitHub OAuth token (gho_*) that only works on GHE.
        # In both cases we select the right pool entry for the target model.
        _source = str(creds.get("source", "")).strip()
        _token = str(creds.get("api_key", "")).strip()
        if provider == "copilot" and (
            _source.startswith("credential_pool:")
            or _token.startswith("gho_")
        ):
            ...
            if pool_entry is not None:
                ...
                if pool_key:
                    ...
                    if pool_key.startswith("gho_"):
                        from hermes_cli.copilot_auth import get_copilot_api_token
                        pool_key = get_copilot_api_token(pool_key, base_url=pool_base) or pool_key
                    api_key = pool_key
                if pool_base:
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            # GitHub Enterprise Copilot (copilot-api.*) also needs headers
            from hermes_cli.copilot_auth import copilot_request_headers

            headers.update(copilot_request_headers(
                is_agent_turn=True, is_vision=is_vision, base_url=base_url
            ))
        client = OpenAI(api_key=api_key, base_url=base_url,
                        **({"default_headers": headers} if headers else {}))
        # Workaround: manually set headers on httpx client if default_headers didn't work
        # (some OpenAI SDK versions don't properly propagate default_headers)
        if headers and hasattr(client, "_client"):
            for k, v in headers.items():
                client._client.headers.set(k, v)
            # Debug log to verify headers are set
            _header_keys = list(headers.keys())
            print(f"[DEBUG] resolve_provider_client: manually set headers {len(_header_keys)} on httpx client for base_url={base_url}", flush=True)
            print(f"[DEBUG] Headers now: {list(client._client.headers.keys())}", flush=True)
        ...
        # NOTE: gpt-5 models (except gpt-5-mini) ALWAYS need Responses API for GHE.
        # Don't skip this even when raw_codex=True — raw_codex just means "don't wrap
        # in CodexAuxiliaryClient for OAuth token handling", but GHE still needs the
        # Responses API adapter for gpt-5 models.
        _skip_responses_wrapper = raw_codex and not base_url_host_matches(base_url, "copilot-api")
        if provider == "copilot" and final_model and not _skip_responses_wrapper:
            try:
                from hermes_cli.models import _should_use_copilot_responses_api
                _use_responses = _should_use_copilot_responses_api(final_model)
                print(f"[DEBUG] resolve_provider_client: provider=copilot final_model={final_model} _should_use_responses={_use_responses} _skip_responses_wrapper={_skip_responses_wrapper}", flush=True)
                if _use_responses:
                    print(f"[DEBUG] resolve_provider_client: wrapping in CodexAuxiliaryClient for model={final_model} (Responses API)", flush=True)
                    client = CodexAuxiliaryClient(client, final_model)
                    print(f"[DEBUG] resolve_provider_client: wrapped client type={type(client).__name__}", flush=True)
            except ImportError:
                pass
...
def get_compression_client(
    main_runtime: Optional[Dict[str, Any]] = None,
) -> Tuple[Optional[Any], Optional[str], Optional[str]]:
    """Try compression-suitable providers in priority order.

    Iterates ``_COMPRESSION_PROVIDER_CHAIN`` (OpenRouter → Nous → custom →
    api-key), skipping providers that are the main_runtime's provider (to avoid
    retrying a provider that is already known to fail for compression).

    Returns (client, model, provider_label) or (None, None, "") if no provider
    is available.

    Unlike ``get_text_auxiliary_client()`` which uses the main runtime's
    provider first, this function explicitly excludes Copilot Enterprise
    because Copilot returns "unauthorized" for compression/summary tasks.
    """
    main_provider = None
    if main_runtime:
        main_provider = (main_runtime.get("provider") or "").strip().lower()

    # Providers that are known to return "unauthorized" for compression tasks
    # even with valid credentials — skip them entirely for compression.
    COMPRESSION_EXCLUDED_PROVIDERS = {"copilot", "github-copilot", "github-copilot-enterprise", "copilot-enterprise"}

    tried = []
    for label, try_fn in _COMPRESSION_PROVIDER_CHAIN:
        if label in COMPRESSION_EXCLUDED_PROVIDERS:
            continue
        if main_provider and label == main_provider:
            # Don't retry the main runtime provider for compression — if it
            # returned an auth/transient error we already tried it above.
            # But if the main provider is "openrouter" or "nous" (generic),
            # still try because the error might have been transient/rate-limit.
            if label not in ("openrouter", "nous"):
                logger.debug(
                    "get_compression_client: skipping main provider %s (compression excluded)",
                    label,
                )
                continue
        ...
        try:
            ...
            if client is not None:
                logger.info(
                    "get_compression_client: using %s (%s)",
            ...
            logger.debug(
                "get_compression_client: %s unavailable (%s); trying next",
                label, exc,
            )
            tried.append(label)

    logger.warning(
        "get_compression_client: no provider available for compression "
        "(tried: %s). Summary generation will fail.",
...
def extract_content_or_reasoning(response) -> str:
    ...
    Resolution order:
      1. ``message.content`` as plain string — strip inline think/reasoning
         blocks, return remaining non-whitespace text.
      2. ``message.content`` as typed-block list (DeepSeek V4 Pro / Anthropic
         style) — concatenate ``output``/``text`` blocks; thinking blocks are
         intentionally skipped here (they go into reasoning_content).
      3. ``message.reasoning`` / ``message.reasoning_content`` — direct
         structured reasoning fields (DeepSeek, Moonshot, Novita, etc.).
      4. ``message.reasoning_details`` — OpenRouter unified array format.
    ...
    if isinstance(raw_content, list):
        # Typed-block list (e.g. DeepSeek V4 Pro thinking mode):
        #   [{"type": "thinking", "thinking": "..."}, {"type": "output", "text": "..."}]
        # Collect text from output/text blocks; skip thinking blocks (those
        # belong in reasoning_content, extracted separately).
        text_parts = []
    ...
    # Typed-block list: also extract thinking blocks as reasoning fallback
    if not reasoning_parts and isinstance(raw_content, list):
        for block in raw_content:
            if isinstance(block, dict) and block.get("type") == "thinking":
                thinking_text = (block.get("thinking") or block.get("text") or "").strip()
